...
public class State {
	...
	//make a move based on the move index - its order in the legalMoves list
	public State getStateAfterMove(int move) {
		return getStateAfterMove(legalMoves[nextPiece][move]);
	}
	
	//make a move based on an array of orient and slot
	public State getStateAfterMove(int[] move) {
		return getStateAfterMove(move[ORIENT],move[SLOT]);
	}
	
	public State getStateAfterMove(int orient, int slot) {
		State s = new State();
		s.lost = lost;
		s.turn = turn;
		s.cleared = cleared;
	...
	public float[] getBasisForMove(int move) {
		return getBasisForMove(legalMoves[nextPiece][move]);
	}
	
	//make a move based on an array of orient and slot
	public float[] getBasisForMove(int[] move) {
		return getBasisForMove(move[ORIENT],move[SLOT]);
	}
	
	public float[] getBasisForMove(int orient, int slot) {
		State newState = getStateAfterMove(orient, slot);
		
		int oldTop = 0, newTop = 0;
		int oldHoles = 0, newHoles = 0;
		int oldTotal = 0, newTotal = 0;
		int oldDiff = 0, newDiff = 0;
		int prevOldTop = 0, prevNewTop = 0; 
		for (int i = 0; i < COLS; ++i) {
			int oldTopI = top[i], newTopI = newState.top[i], j;
			oldTop = oldTop > oldTopI ? oldTop : oldTopI;
			newTop = newTop > newTopI ? newTop : newTopI;
			
			for (j = oldTopI - 1; j >= 0; --j)
				if (field[j][i] == 0)
					++oldHoles;
			
			for (j = newTopI - 1; j >= 0; --j)
				if (newState.field[j][i] == 0)
					++newHoles;
			...
			if (i != 0) {
				oldDiff += Math.abs(prevOldTop - oldTopI);
				newDiff += Math.abs(prevNewTop - newTopI);
			}
